    ...
    public List<ReportedWorkloade> getReportedWorkloadeList() {
        ...
        for (int i = 5; i < this.getLastDataRowNum(); i++) {
        	...
        	if(Cell.CELL_TYPE_NUMERIC == row.getCell(1).getCellType()){
        	    Reflections.setFieldValue(rw, "user", UserUtils.getByLoginName(String.valueOf((int)row.getCell(1).getNumericCellValue())));
        	}else if(Cell.CELL_TYPE_STRING == row.getCell(1).getCellType()){
        	    Reflections.setFieldValue(rw, "user", UserUtils.getByLoginName(row.getCell(1).getStringCellValue()));
        	}
