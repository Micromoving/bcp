 ...
 */
package cn.micromoving.bcp.modules.act.service;
...
import cn.micromoving.bcp.modules.act.dao.ActDao;
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
		...
		// =============== 等待签收的任务  ===============
		TaskQuery toClaimQuery = taskService.createTaskQuery().taskCandidateGroup(userId)
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	...
	public Page<Act> historicList(Page<Act> page, Act act){
		...
		HistoricTaskInstanceQuery histTaskQuery = historyService.createHistoricTaskInstanceQuery().taskCandidateGroup(userId);
		// 设置查询条件
		if (StringUtils.isNotBlank(act.getProcDefKey())){
		...
		//按task执行的结束时间倒序排序
		histTaskQuery.orderByHistoricTaskInstanceEndTime().desc();
		// 查询总数
		page.setCount(histTaskQuery.count());
		act.setPage(page);
		
		// 查询列表
		List<HistoricTaskInstance> histList = historyService.createHistoricTaskInstanceQuery().taskCandidateGroup(userId).orderByHistoricTaskInstanceEndTime().desc().list();
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
		...
		//指定分页，每页值显示30个
		int begin = (page.getPageNo()-1)*30;
		int end = begin +30;
		//如果为末页，则重新更改end值
		if(end > acts.size()){
		    end =acts.size();
		}
		List<Act> act2 = new ArrayList<>();
...
//		}
	...
	public void completeFirstTask(String procInsId, String comment, String title, Map<String, Object> vars){
		...
		Task task = taskService.createTaskQuery().taskCandidateGroup(userId).processInstanceId(procInsId).active().singleResult();
		if (task != null){
...
//	}
	...
	public String startProcess(String procDefKey, String businessTable,
		 ...
		 * 启动流程
		 */
		String procInsId = this.startProcess(procDefKey,
