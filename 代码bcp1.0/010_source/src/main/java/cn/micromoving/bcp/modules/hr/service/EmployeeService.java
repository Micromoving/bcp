...
public class EmployeeService extends CrudService<EmployeeDao, Employee>{
    ...
    public List<Employee> findAllList(Employee employee) {
        employee.getSqlMap().put("dsf", dataScopeFilter(employee.getCurrentUser(), "o", "a"));
        List<Employee> employeeList = dao.findAllList(employee);
        return employeeList;
    }

    public Page<Employee> findEdu(Page<Employee> page, Employee employee) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
	    employee.getSqlMap().put("dsf", dataScopeFilter(employee.getCurrentUser(), "o", "a"));
		// 设置分页参数
	    employee.setPage(page);
		// 执行分页查询
		page.setList(employeeDao.findEdu(employee));
		return page;
	}
