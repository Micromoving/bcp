...
public class TrainExperienceService extends
	...
	public Page<TrainExperience> find(Page<TrainExperience> page, TrainExperience trainExperience) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		trainExperience.getSqlMap().put("dsf", dataScopeFilter(trainExperience.getCurrentUser(), "o", "a"));
		// 设置分页参数
		trainExperience.setPage(page);
		// 执行分页查询
		page.setList(trainExperienceDao.findList(trainExperience));
		return page;
	}
...
//
//	}
	public Page<TrainExperience> findEmployee(Page<TrainExperience> page, TrainExperience trainExperience) {
		// 生成数据权限过滤条件（dsf为dataScopeFilter的简写，在xml中使用 ${sqlMap.dsf}调用权限SQL）
		trainExperience.getSqlMap().put("dsf", dataScopeFilter(trainExperience.getCurrentUser(), "o", "a"));
		// 设置分页参数
		trainExperience.setPage(page);
