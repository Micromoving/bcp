...
<script type="text/javascript">
	...
	function performanceOnchange(data){
		for(var i=0;i<${list.size()};i++){
			if(data=="1"){
				$("#performanceSalary"+i).val($("#stdSal"+i).text());
			}else if(data=="2"){
									...
									<div class="media stream">
										<form:form id="batchForm"
											...
											<div class="control-group" style="margin-bottom:6px;">
												...
												&nbsp;&nbsp;数据来源
												<form:select
													onchange="performanceOnchange($(this).val())"
													path="dataFrom" class="span3">
													<form:option label="无" value="0" />
													<form:option label="应发绩效" value="1" />
													<form:option label="上月所发绩效" value="2" />
												</form:select>
											</div>
													...
													<tbody>
														<c:forEach var="data" items="${list}" varStatus="status">
															<tr>
																...
																<td id="stdSal${status.index}">${data.stdSal}</td>
																<td id="latestSal${status.index}">${data.latestSal}</td>
																<td><input type="text" id="performanceSalary${status.index}"
																	...
																	name="dataIdList[${status.index}]" value="${data.id}">&nbsp;元</td>
															</tr>
														</c:forEach>
													</tbody>
												</table>
