							...
							<div class="module-body">
								<div class="clearfix">
									...
									<c:if test="${reportRecored.dataClassification  eq '3'}">
										<a
											href="${ctx}/hr/dutyDetail/list?procDefId=${process.id}&reportRecord.dataClassification=3"
											class="btn btn-primary pull-right">上报</a>
									</c:if>
									<c:if
										test="${reportRecored.dataClassification  eq '2' && flag ne '0'}">
										<a
											href="${ctx}/hr/reportedWorkloade/list?procDefId=${process.id}"
											class="btn btn-primary pull-right">上报</a>
									</c:if>
									<c:if
										test="${reportRecored.dataClassification  eq '2'&& flag eq '0' }">
										<a
											href="${ctx}/hr/reportedWorkloade/list?procDefId=${process.id}&flag=0"
											class="btn btn-primary pull-right">生成本月院汇总结果 </a>
									</c:if>
									...
									<c:if test="${reportRecored.dataClassification   eq '6' }">
										<a
											href="${ctx}/hr/reportPerformance/list?postType=3&reportRecord.dataClassification=6&procDefId=${process.id}"
											class="btn btn-primary pull-right">上报</a>
									</c:if>
											...
											<table
												...
												<thead>
													<tr>
														...
														<th>上传者</th>
														<c:if test="${empty reportRecord.dataClassification}">
															<shiro:hasAnyPermissions name="hr:reportrecord:list">
																<th>所属工资实例</th>
												...
												<tbody>
													<c:forEach var="data" items="${records}" varStatus="status">
														<tr>
															...
															<c:if test="${empty reportRecord.dataClassification}">
																<shiro:hasAnyPermissions name="hr:reportrecord:list">
																	<td><c:if test="${empty data.salaryInstance.id}">
																			<input type="hidden"
															...
															<td><c:if
																	test="${('1' eq data.dataState || '4' eq data.dataState) &&  empty data.act.procInsId}">草稿状态</c:if>
																<c:if
																	test="${'2' eq data.dataState || '5' eq data.dataState || not empty data.act.procInsId}">${data.act.taskName}</c:if>
																<c:if
																	test="${'3' eq data.dataState && data.dataClassification eq '2'}">教务处统计审核</c:if>

																&nbsp;<c:if test="${data.dataClassification   eq '1' }">
																	<a
																		href="${ctx}/hr/reportPerformance/form?procDefId=${process.id}&reportRecord.id=${data.id}&postType=2">详情</a>
																</c:if> <c:if
																	test="${data.dataClassification   eq  '2' && data.flag ne '0'}">
																	<a
																		href="${ctx}/hr/reportedWorkloade/form?reportRecord.id=${data.id}">详情</a>
																	<a
																		href="${ctx}/hr/reportedWorkloade/export?reportRecord.id=${data.id}">下载</a>
																</c:if> <c:if
																	test="${data.dataClassification   eq  '2' && data.flag eq '0'}">
																	<a
																		href="${ctx}/hr/reportedWorkloade/form?reportRecord.id=${data.id}&flag=0">详情</a>
																	<a
																		href="${ctx}/hr/reportedWorkloade/export?reportRecord.id=${data.id}">下载</a>
																...
																</c:if> <c:if test="${data.dataClassification   eq '6' }">
																	<a
																		href="${ctx}/hr/reportPerformance/form?procDefId=${process.id}&reportRecord.id=${data.id}&postType=3">详情</a>
																</c:if></td>
														</tr>
													</c:forEach>
												</tbody>
