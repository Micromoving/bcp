...
import cn.micromoving.bcp.common.security.Digests;
import cn.micromoving.bcp.common.security.shiro.session.SessionDAO;
import cn.micromoving.bcp.common.service.BaseService;
import cn.micromoving.bcp.common.service.ServiceException;
import cn.micromoving.bcp.common.utils.CacheUtils;
import cn.micromoving.bcp.common.utils.Encodes;
import cn.micromoving.bcp.common.utils.StringUtils;
import cn.micromoving.bcp.common.web.Servlets;
...
import cn.micromoving.bcp.modules.sys.security.SystemAuthorizingRealm;
import cn.micromoving.bcp.modules.sys.utils.LogUtils;
...
public class SystemService extends BaseService implements InitializingBean {
	...
	public void saveUser(User user) {
		/* 如果ID为空，表示是create操作。否则是update操作。 */
		if (StringUtils.isBlank(user.getId())) {
			user.preInsert();
			userDao.insert(user);
			/* 添加新用户后，如果用户类型为教师，在教师表中级联添加一条记录。 */
			if ("1".equals(user.getUserType())) {
				/* 教师主键与用户表一一对应。教师表中的新用户，主键关联。 */
				Employee entity = new Employee(user.getId());

				/* 教师工号与用户的登录ID关联。 */
				entity.setUserNo(user.getLoginName());
			...
			user.preUpdate();
			userDao.update(user);
			/* 更新用户后，如果用户类型为教师，做及联更新。 */
			if ("1".equals(user.getUserType())) {
				Employee entity = employeeDao.get(user.getId());
				/* 如果用户的登录名发生变化，则将教师信息中的工号作出对应变化。 */
				if (!entity.getUserNo().equals(user.getLoginName())) {
					entity.setUserNo(user.getLoginName());
	...
	public void updatePasswordById(String id, String loginName,
		...
		// 清除用户缓存
		if (null == loginName) {
			loginName = userDao.get(id).getLoginName();
		}
	...
	/**
	 * 取得当前用户权限。管理员：全部系统权限。其它用户：对应的权限。
	 * 
	 * @return 当前用户权限
	 */
	...
	public static boolean printKeyLoadMessage() {
		...
		sb.append("\r\n    欢迎使用 " + Global.getConfig("productName")
				+ "  - Powered By http://www.micromoving.cn\r\n");
		sb.append("\r\n======================================================================\r\n");
