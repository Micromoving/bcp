						...
						<div class="module">
							...
							<div class="module-body">
								<div class="clearfix">
									...
									<c:if test="${reportRecored.dataClassification  eq '2' && flag ne '0'}">
										<a
											href="${ctx}/hr/reportedWorkloade/list?procDefId=${process.id}"
											class="btn btn-primary pull-right">上报</a>
									</c:if>
									<c:if test="${reportRecored.dataClassification  eq '2'&& flag eq '0' }">
										<a
												...
												<tbody>
													<c:forEach var="data" items="${page.list}"
														...
														<tr>
															...
															<td>
															<c:if
																	test="${('1' eq data.dataState || '4' eq data.dataState) &&  empty data.act.procInsId}">草稿状态</c:if> <c:if
																	test="${'2' eq data.dataState || '5' eq data.dataState || not empty data.act.procInsId}">${data.act.taskName}</c:if>
																<c:if test="${'3' eq data.dataState ||'6' eq data.dataState }">已完成</c:if> &nbsp;<c:if
																...
																</c:if> <c:if test="${data.dataClassification   eq  '2' && data.flag ne '0'}">
																	<a
																...
																</c:if><c:if test="${data.dataClassification   eq  '2' && data.flag eq '0'}">
																	<a
