/*!
 * Copyright &copy; 2012-2014 <a href="https://github.com/bcp/jeesite">bcp</a> All rights reserved.
 * 
 * 通用公共方法
 * @author songcm
 * @version 2016-4-29
...
function parseJson(id, xmlDocument) {
	var subSelect = document.getElementById(id);
	...
	for (var i = 0; i < size; i++) {
		...
		var optionElement = document.createElement("OPTION"); // 创建option对象
		optionElement.text = json[i].label;
		// 指定新创建元素的text属性值
		optionElement.value = json[i].value;

		subSelect.appendChild(optionElement);
	}
}

function selectChange(value, url, subId) {

	var subSelect = document.getElementById(subId);

	// 删除多余的城市，从最后一个开始删除
	var size = subSelect.options.length;
	if (size > 1) {
	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				parseJson(subId, xhr.responseText);
...
function ajaxPost(url, data, outId) {
...
function ajaxSubmit(url, form, outId) {
...
function input_numericCheck(stringText) {

	if ((stringText == "") || (stringText.search(/\D/) == -1)) {
		return true;
	} else {
		return false;
	}
}
function page(n, s, url) {
	if (n)
		$("#pageNo").val(n);
	if (s)
		$("#pageSize").val(s);
