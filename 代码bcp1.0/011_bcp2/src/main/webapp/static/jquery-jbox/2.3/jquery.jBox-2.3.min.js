(function(a) {
	a.jBox = function(b, c) {
		...
		j
				.push('<a class="jbox-close" title="'
						...
						+ (6 + c.border) + "px; right:" + (6 + c.border)
		...
		if (!f.isObject) {
			...
			case "IFRAME":
				f.html = k;
				if (b.indexOf("#") == -1) {
					f.url = b + (b.indexOf("?") == -1 ? "?___t" : "&___t")
							+ Math.random()
				} else {
					var N = b.split("#");
					f.url = N[0]
							+ (N[0].indexOf("?") == -1 ? "?___t" : "&___t")
							+ Math.random() + "#" + N[1]
				}
				break
			}
		...
		var F = function(N) {
			...
			if (c.dragLimit) {
				...
				var S = document.documentElement.clientHeight
						- N.data.target.height() - 1;
				var T = 1;
				var U = document.documentElement.clientWidth
						- N.data.target.width() - 1;
				if (Q < R) {
					Q = R + (c.dragClone ? 2 : 0)
				}
				if (Q > S) {
					...
					if (tip && tip.next == true) {
						...
						if (N == o.offset().top) {
							...
							o.find("#jbox-content").html(
									tip.options.content.substr(5)).end().css({
								left : ((l.width() - o.outerWidth()) / 2)
							}).animate({
								top : N,
								opacity : 0.1
	...
	a.jBox.prompt = function(b, c, d, e) {
		...
		var g = e.icon == "none" ? "height:auto;"
				: "min-height:30px;"
						+ ((a.browser.msie && parseInt(a.browser.version) < 7) ? "height:auto !important;height:100%;_height:30px;"
								: "height:auto;");
		var h = [];
		h.push("html:");
		h.push('<div style="margin:10px;' + g + "padding-left:"
				+ (e.icon == "none" ? 0 : 40) + 'px;text-align:left;">');
	...
	a.jBox.tip = function(b, c, d) {
		...
		e.push('<span class="jbox-icon jbox-icon-' + d.icon
				+ '" style="position:absolute;top:' + (4 + d.border)
				+ "px;left:" + (4 + d.border)
				+ 'px; width:32px; height:32px;"></span>');
		e.push(d.content);
		e.push("</div>");
