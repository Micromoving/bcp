...
bcp是基于多个优秀的开源项目，高度整合封装而成的高效，高性能，强安全性的**开源**Java EE快速开发平台。

bcp本身是以Spring Framework为核心容器，Spring MVC为模型视图控制器，MyBatis为数据访问层，
Apache Shiro为权限授权层，Ehcahe对常用数据进行缓存，Activit为工作流引擎。

bcp主要定位于企业信息化领域，已内置企业信息化系统的基础功能和高效的**代码生成**工具，
包括：系统权限组件、数据权限组件、数据字典组件、核心工具组件、视图操作组件、工作流组件、代码生成等。
前端界面风格采用了结构简单、性能优良、页面美观大气的Twitter Bootstrap页面展示框架。
采用分层设计、双重验证、提交数据安全编码、密码加密、访问验证、数据权限验证。
使用Maven做项目管理，提高项目的易开发性、扩展性。

bcp目前包括以下三大模块，系统管理（SYS）模块、
...
2. 修改src\main\resources\bcp.properties文件中的数据库设置参数。
...
6. 最高管理员账号，用户名：songcm 密码：admin

## 常见问题

1. 有时出现文字乱码：修改Tomcat的server.xml文件的Connector项，增加URIEncoding="UTF-8"
2. 用一段时间提示内存溢出，请修改JVM参数：-Xmx512m -XX:MaxPermSize=256m
3. 为什么新建菜单后看不到新建的菜单？因为授权问题，菜单管理只允许最高管理员账号管理（最高管理员默认账号：songcm 密码：admin）。

## 更多文档

* <https://github.com/songcm/bcp/tree/master/doc>

## 如何交流、反馈、参与贡献？

* QQ Group：127515876 &nbsp; 209330483 &nbsp; 223507718
* E-mail：songcm@163.com
* GitHub：<https://github.com/songcm/bcp>
* 开源中国：<http://git.oschina.net/songcm/bcp>
* 官方网址：<http://bcp.com>  论坛：<http://bbs.bcp.com>
* 捐赠支持bcp（支付宝）：[songcm@163.com](http://bcp.com/donation.html)
...
* 多数据库支持：有些人说MyBatis对多数据库支持困难，我认为这个不是问题，虽说目前bcp仅提供对MySql或Oracle
数据库的支持，但对于支持其它数据库的改动也不是很麻烦，SQL是被专门写在XML中，对于大多数SQL来说都是通用的，
对于不同的数据库可通过dbName区分和修改各别的SQL片段即可。

上述纯属个人观点，欢迎指正（songcm@163.com）。

如果你仍然喜欢Hibernate，请点击[下载bcp Hibernate版本](https://github.com/songcm/bcp/tree/master_hibernate)。
