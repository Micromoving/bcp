...
public class UploadUtils {
	...
	public String uploadFile(HttpServletRequest request,MultipartFile file)  {
		...
		String fileName = this.getUniqueFileName()+"."+getExtName(file.getOriginalFilename());
		// 验证
		try{
			info = this.validateFields(request);
			if("true".equals(info)){
				System.out.println(request.getSession().getServletContext().getRealPath("/") +this.basePath+fileName);
				file.transferTo(new File(request.getSession().getServletContext().getRealPath("/") +this.basePath+fileName));
			}else {
				return null;
			}
