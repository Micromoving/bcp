...
public class EmpPerformanceLevelService extends
		CrudService<EmpPerformanceLevelDao, EmpPerformanceLevel> {
	...
	public void batchSave(EmpPerformanceLevel empPerformanceLevel) {
		...
		/* 保存上报工作量详情的n条记录 */
		for (int index = 0; index < empPerformanceLevel.getEmployeeIdList()
				.size(); index++) {
			...
			String performanceLevel = empPerformanceLevel
					.getPerformanceLevelList().get(index);
	...
	public boolean ccTime(EmpPerformanceLevel empPerformanceLevel)
			throws ParseException {
		boolean data = false;
		SimpleDateFormat sim = new SimpleDateFormat("yyyyMM");
		Date date1 = sim.parse(empPerformanceLevel.getReportRecord()
				.getYearMonth());
		String dates1 = String.format("%tY", date1);
		List<ReportRecord> list = reportRecordDao.findList(empPerformanceLevel
				.getReportRecord());
