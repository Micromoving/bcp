					...
					'tinymce.util.URI',
					{
						URI : function(u, s) {
							...
							if (u.indexOf(':/') === -1 && u.indexOf('//') !== 0)
								u = (s.base_uri.protocol || 'http')
										+ '://mce_host'
										+ t.toAbsPath(s.base_uri.path, u);
							u = u.replace(/@@/g, '(mce_at)');
							u = /^(?:(?![^:@]+:[^:@\/]*@)([^:\/?#.]+):)?(?:\/\/)?((?:(([^:@]*):?([^:@]*))?@)?([^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/
									.exec(u);
							each([ "source", "protocol", "authority",
			...
			function ready() {
				if (!o.async || x.readyState == 4 || c++ > 10000) {
					if (o.success && c < 10000 && x.status == 200)
						o.success.call(o.success_scope, '' + x.responseText, x,
								o);
					else if (o.error)
			...
			.create(
					'tinymce.dom.DOMUtils',
					{
						...
						DOMUtils : function(d, s) {
							...
							if (tinymce.isIE6) {
								try {
									d.execCommand('BackgroundImageCache',
											false, true);
								} catch (e) {
									t.cssFlicker = true;
								}
							...
							if (is(f, 'string')) {
								...
								f = function(n) {
									...
									each(
											...
											function(v) {
												if (n.nodeType == 1
														&& ((se.strict && n.nodeName
																.toUpperCase() == v) || n.nodeName
																.toUpperCase() == v)) {
													s = true;
													return false;
												}
						...
						add : function(p, n, a, h, c) {
							...
							return this.run(p, function(p) {
								var e, k;
								e = is(n, 'string') ? t.doc.createElement(n)
										: n;
								t.setAttribs(e, a);
								if (h) {
									if (h.nodeType)
						...
						create : function(n, a, h) {
							return this.add(this.doc.createElement(n), n, a, h,
									1);
						},
						...
						getPos : function(n) {
							...
							if (n && isIE) {
								...
								x = t.getStyle(t.select('html')[0],
										'borderWidth');
								x = (x == 'medium' || t.boxModel && !t.isIE6)
										&& 2 || x;
							...
							r = n;
							while (r) {
								if (!/^table-row|inline.*/i.test(t.getStyle(r,
										"display", 1))) {
									x -= r.scrollLeft || 0;
									y -= r.scrollTop || 0;
								}
						...
						isBlock : function(n) {
							if (n.nodeType && n.nodeType !== 1)
								return false;
							n = n.nodeName || n;
							return /^(H[1-6]|HR|P|DIV|ADDRESS|PRE|FORM|TABLE|LI|OL|UL|TD|CAPTION|BLOCKQUOTE|CENTER|DL|DT|DD|DIR|FIELDSET|NOSCRIPT|NOFRAMES|MENU|ISINDEX|SAMP)$/
									.test(n);
						},
						...
						toHex : function(s) {
							var c = /^\s*rgb\s*?\(\s*?([0-9]+)\s*?,\s*?([0-9]+)\s*?,\s*?([0-9]+)\s*?\)\s*$/i
									.exec(s);
							function hex(s) {
					...
					'static tinymce.dom.Event',
					{
						...
						_wait : function() {
							...
							if (isIE && document.location.protocol != 'https:') {
								...
								Event._add(window, 'DOMContentLoaded',
										Event._pageInit, Event);
								if (isIE || isWebKit) {
		...
		update : function(k) {
			...
			if (tinymce.isIE6 && t.settings.blocker) {
				...
				if (!t.blocker) {
					...
					b = dom.add(t.settings.container || dom.getRoot(),
							'iframe', {
								id : t.blocker,
				...
				dom.setStyle(b, 'zIndex',
						parseInt(t.getStyle('zIndex', 1) || 0) - 1);
			}
		}
	});
})();
			...
			.create(
					'tinymce.dom.Selection',
					{
						Selection : function(dom, win, serializer) {
							...
							each([ 'onBeforeSetContent', 'onBeforeGetContent',
						...
						getContent : function(s) {
							var t = this, r = t.getRng(), e = t.dom
									.create("body"), se = t.getSel(), wb, wa, n;
							s = s || {};
							wb = wa = '';
							s.get = true;
							...
							if (r.cloneContents) {
								...
								e.innerHTML = r.item ? r.item(0).outerHTML
										: r.htmlText;
							else
							...
							if (/\s+$/.test(e.innerHTML))
								wa = ' ';
							s.getInner = true;
							s.content = t.isCollapsed() ? '' : wb
									+ t.serializer.serialize(e, s) + wa;
						...
						getBookmark : function(si) {
							var t = this, r = t.getRng(), tr, sx, sy, vp = t.dom
									.getViewPort(t.win), e, sp, bp, le, c = -0xFFFFFF, s, ro = t.dom
									.getRoot(), wb = 0, wa = 0, nv;
							sx = vp.x;
							sy = vp.y;
							if (si == 'simple')
								return {
							...
							if (isIE) {
								if (r.item) {
									e = r.item(0);
									each(t.dom.select(e.nodeName), function(n,
											i) {
										if (e == n) {
											sp = i;
											return false;
							...
							function getPos(r, sn, en) {
								var w = t.dom.doc.createTreeWalker(r,
										NodeFilter.SHOW_TEXT, null, false), n, p = 0, d = {};
							...
							if (s.anchorNode == s.focusNode
								...
								trimNl(s.anchorNode.nodeValue || '').replace(
										/^\s+/, function(a) {
											wb = a.length;
										});
								return {
									start : Math.max(e.start + s.anchorOffset
											- wb, 0),
									end : Math.max(e.end + s.focusOffset - wb,
											0),
									scrollX : sx,
									scrollY : sy,
								...
								return {
									start : Math.max(e.start + r.startOffset
											- wb, 0),
						...
						moveToBookmark : function(b) {
							var t = this, r = t.getRng(), s = t.getSel(), ro = t.dom
									.getRoot(), sd, nvl, nv;
							function getPos(r, sp, ep) {
								var w = t.dom.doc.createTreeWalker(r,
										NodeFilter.SHOW_TEXT, null, false), n, p = 0, d = {}, o, v, wa, wb;
								while ((n = w.nextNode()) != null) {
									wa = wb = 0;
									nv = n.nodeValue || '';
						...
						select : function(n, c) {
							...
							function last(n) {
								...
								if (!n)
									return null;
								w = d.createTreeWalker(n, NodeFilter.SHOW_TEXT,
										null, false);
								while (c = w.nextNode())
									o = c;
								return o;
						...
						getSel : function() {
							...
							return w.getSelection ? w.getSelection()
							...
							try {
								if (s)
									r = s.rangeCount > 0 ? s.getRangeAt(0)
											: (s.createRange ? s.createRange()
													: t.win.document
															.createRange());
							} catch (ex) {
							}
							if (!r)
								r = isIE ? t.win.document.body
										.createTextRange() : t.win.document
										.createRange();
							return r;
						},
						setRng : function(r) {
						...
						getNode : function() {
							...
							if (!isIE) {
								...
								if (!r.collapsed) {
									if (tinymce.isWebKit && s.anchorNode
...
(function() {
	...
	function getIEAtts(n) {
		...
		if (n.nodeName == 'OBJECT')
			return n.attributes;
		n.cloneNode(false).outerHTML.replace(/([a-z0-9\:\-_]+)=/gi, function(a,
				b) {
			o.push({
				specified : 1,
				nodeName : b
					...
					'tinymce.dom.Serializer',
					{
										...
										.add(function(se, o) {
											each(
													...
													function(e) {
														var pa = t.dom
																.getParent(e,
																		'H1,H2,H3,H4,H5,H6,P'), pa2, n, tm, pl = [], i, ns;
														if (pa) {
															pa2 = pa
																	.cloneNode(false);
															pl.push(e);
							...
							if (o.format == 'html') {
								...
								if (!o.set) {
									h = h
											.replace(
													/<p>\s+<\/p>|<p([^>]+)>\s+<\/p>/g,
													s.entity_encoding == 'numeric' ? '<p$1>&#160;</p>'
															: '<p$1>&nbsp;</p>');
									...
									if (s.apply_source_formatting
											&& s.indent_mode == 'simple') {
										h = h
												.replace(
						...
						_serializeNode : function(n, inn) {
							...
							if (!s.node_filter || s.node_filter(n)) {
								...
								case 1:
									if (n.hasAttribute ? n
											.hasAttribute('mce_bogus') : n
											.getAttribute('mce_bogus'))
										return;
									iv = false;
									hc = n.hasChildNodes();
									nn = n.getAttribute('mce_name')
									...
									if (nn.indexOf('mce:') === 0)
										nn = nn.substring(4);
									if (!t.validElementsRE.test(nn)
											|| (t.invalidElementsRE && t.invalidElementsRE
													.test(nn)) || inn) {
										iv = true;
										break;
									}
									if (isIE) {
										...
										if (n.nodeName === 'BR'
												&& n.getAttribute('type') == '_moz')
											return;
									}
									if (t.childRules) {
										if (t.parentElementsRE
												.test(t.elementName)) {
											if (!t.childRules[t.elementName]
													.test(nn)) {
												iv = true;
												break;
											}
									...
									if (ru.padd) {
										if (hc && (cn = n.firstChild)
												&& cn.nodeType === 1
												&& n.childNodes.length === 1) {
											if (cn.hasAttribute ? cn
						...
						_protect : function(o) {
							...
							each(o.patterns, function(p) {
								o.content = dec(enc(o.content).replace(
										p.pattern,
										function(x, a, b, c) {
											b = dec(b);
											if (p.encode)
						...
						_unprotect : function(h, o) {
							h = h.replace(/\<!--mce:([0-9]+)--\>/g, function(a,
									b) {
						...
						_setup : function() {
							...
							if (s.invalid_elements)
								t.invalidElementsRE = new RegExp('^('
										+ wildcardToRE(s.invalid_elements
												.replace(/,/g, '|')
												.toLowerCase()) + ')$');
				...
				if (!Event.domLoaded && !t.settings.strict_mode) {
					...
					if (cb || o.func) {
						...
						if (tinymce.isIE)
							...
							ol = ' onload="';
						ol += 'tinymce.dom.ScriptLoader._onLoad(this,\'' + u
								+ '\',' + ix + ');"';
					}
					document.write('<script type="text/javascript" src="'
			...
			if (s != this.disabled) {
				...
				if (e && this.settings.unavailable_prefix) {
					if (s) {
						...
						e.title = this.settings.unavailable_prefix + ": "
								+ e.title;
					} else
			...
			.create(
					'tinymce.ui.DropMenu:tinymce.ui.Menu',
					{
						...
						createMenu : function(s) {
							...
							m = new tinymce.ui.DropMenu(s.id || DOM.uniqueId(),
									s);
							m.onAddItem.add(t.onAddItem.dispatch, t.onAddItem);
							return m;
						},
						update : function() {
							var t = this, s = t.settings, tb = DOM.get('menu_'
									+ t.id + '_tbl'), co = DOM.get('menu_'
									+ t.id + '_co'), tw, th;
							tw = s.max_width ? Math.min(tb.clientWidth,
									s.max_width) : tb.clientWidth;
							th = s.max_height ? Math.min(tb.clientHeight,
									s.max_height) : tb.clientHeight;
							if (!DOM.boxModel)
								t.element.setStyles({
									width : tw + 2,
									height : th + 2
						...
						showMenu : function(x, y, px) {
							var t = this, s = t.settings, co, vp = DOM
									.getViewPort(), w, h, mx, my, ot = 2, dm, tb, cp = t.classPrefix;
							t.collapse(1);
							if (t.isMenuVisible)
								return;
							if (!t.rendered) {
								co = DOM.add(t.settings.container, t
										.renderNode());
							...
							t.element.update();
							t.isMenuVisible = 1;
							t.mouseClickFunc = Event.add(co, 'click', function(
									e) {
								var m;
								e = e.target;
						...
						_keyHandler : function(e) {
							...
							function focus(d) {
								var i = t._focusIdx + d, e = DOM.select('a',
										'menu_' + t.id)[i];
								if (e) {
									t._focusIdx = i;
									e.focus();
						...
						_add : function(tb, o) {
							...
							if ((n = ro.previousSibling)
									&& DOM.hasClass(n, cp + 'ItemSeparator'))
								DOM.addClass(ro, 'mceFirst');
							if (o.collapse)
					...
					'tinymce.ui.Button:tinymce.ui.Control',
					{
						...
						renderHTML : function() {
							...
							h = '<a id="'
									...
									+ DOM.encode(s.title) + '">';
							if (s.image)
								h += '<img class="mceIcon" src="' + s.image
										+ '" />' + l + '</a>';
							else
								h += '<span class="mceIcon '
										+ s['class']
										+ '"></span>'
										+ (l ? '<span class="' + cp + 'Label">'
												+ l + '</span>' : '') + '</a>';
							return h;
						},
						postRender : function() {
...
(function() {
	...
	tinymce.create('tinymce.ui.ListBox:tinymce.ui.Control', {
		...
		hideMenu : function(e) {
			...
			if (e
					&& e.type == "mousedown"
					&& (e.target.id == t.id + '_text' || e.target.id == t.id
							+ '_open'))
				return;
		...
		postRender : function() {
			...
			Event.add(t.id + '_text', 'focus', function(e) {
				if (!t._focused) {
					t.keyDownHandler = Event.add(t.id + '_text', 'keydown',
							function(e) {
			...
			.create(
					'tinymce.ui.ColorSplitButton:tinymce.ui.SplitButton',
					{
						...
						showMenu : function() {
							...
							Event.add(DOM.doc, 'mousedown', t.hideMenu, t);
							if (t._focused) {
								t._keyHandler = Event.add(t.id + '_menu',
										'keydown', function(e) {
											if (e.keyCode == 27)
												t.hideMenu();
										});
						...
						hideMenu : function(e) {
							...
							if (!e || !DOM.getParent(e.target, function(n) {
								...
								Event.remove(DOM.doc, 'mousedown', t.hideMenu,
										t);
						...
						renderMenu : function() {
							...
							tb = DOM.add(n, 'tbody');
							i = 0;
							each(is(s.colors, 'array') ? s.colors : s.colors
									.split(','), function(c) {
								c = c.replace(/^#/, '');
								if (!i--) {
									tr = DOM.add(tb, 'tr');
							...
							if (s.more_colors_func) {
								...
								Event.add(n, 'click', function(e) {
									s.more_colors_func.call(s.more_colors_scope
											|| this);
									return Event.cancel(e);
								});
							}
						...
						setColor : function(c) {
							var t = this;
							DOM.setStyle(t.id + '_preview', 'backgroundColor',
									c);
							t.value = c;
							t.hideMenu();
			...
			.create(
					'static tinymce.EditorManager',
					{
						...
						preInit : function() {
							...
							tinymce.documentBaseURL = lo.href.replace(
									/[\?#].*$/, '')
									.replace(/[\/\\][^\/]+$/, '');
							...
							tinymce.EditorManager.baseURI = new tinymce.util.URI(
									tinymce.baseURL);
							if (document.domain
									&& lo.hostname != document.domain)
								tinymce.relaxedDomain = document.domain;
							if (!tinymce.relaxedDomain
									&& tinymce.EditorManager.baseURI.host != lo.hostname
									&& lo.hostname)
								document.domain = tinymce.relaxedDomain = lo.hostname
										.replace(/.*\.(.+\..+)$/, '$1');
						...
						init : function(s) {
							...
							function execCallback(se, n, s) {
								...
								return f.apply(s || this, Array.prototype.slice
										.call(arguments, 2));
							}
							;
							s = extend(
							...
							if (!Event.domLoaded && !s.strict_loading_mode) {
								if (s.language)
									sl.add(tinymce.baseURL + '/langs/'
											+ s.language + '.js');
								if (s.theme && s.theme.charAt(0) != '-'
										&& !ThemeManager.urls[s.theme])
									ThemeManager.load(s.theme, 'themes/'
											+ s.theme + '/editor_template'
											+ tinymce.suffix + '.js');
								if (s.plugins) {
									pl = explode(s.plugins);
									if (tinymce.inArray(pl, 'compat2x') != -1)
										PluginManager.load('compat2x',
												'plugins/compat2x/editor_plugin'
														+ tinymce.suffix
														+ '.js');
									each(pl, function(v) {
										if (v && v.charAt(0) != '-'
												&& !PluginManager.urls[v]) {
											if (!tinymce.isWebKit
													&& v == 'safari')
												return;
											PluginManager.load(v, 'plugins/'
													+ v + '/editor_plugin'
													+ tinymce.suffix + '.js');
										}
									});
												...
												case "specific_textareas":
													...
													each(
															DOM
																	.select('textarea'),
															function(v) {
																if (s.editor_deselector
																		&& hasClass(
																				v,
																				s.editor_deselector))
																	return;
																if (!s.editor_selector
																		|| hasClass(
																				v,
																				s.editor_selector)) {
																	e = DOM
			...
			.create(
					'tinymce.Editor',
					{
						Editor : function(id, s) {
							...
							t.plugins = {};
							each([ 'onPreInit', 'onBeforeRenderUI',
						...
						render : function(nst) {
							...
							if (s.add_unload_trigger && !s.ask) {
								t._beforeUnload = tinyMCE.onBeforeUnload
										.add(function() {
							...
							function loadScripts() {
								...
								each(explode(s.plugins), function(p) {
									if (p && p.charAt(0) != '-'
											&& !PluginManager.urls[p]) {
										if (!isWebKit && p == 'safari')
											return;
										PluginManager.load(p, 'plugins/' + p
												+ '/editor_plugin'
												+ tinymce.suffix + '.js');
									}
								});
						...
						init : function() {
							var n, t = this, s = t.settings, w, h, e = t
									.getElement(), o, ti, u, bi, bc, re;
							EditorManager.add(t);
							...
							if (s.custom_undo_redo) {
								t.onExecCommand.add(function(ed, cmd, ui, val,
										a) {
							...
							if (s.render_ui) {
								...
								h = s.height || e.style.height
										|| e.offsetHeight;
								t.orgDisplay = e.style.display;
								re = /^[0-9\.]+(|px)$/i;
								if (re.test('' + w))
									w = Math.max(parseInt(w)
											+ (o.deltaWidth || 0), 100);
								if (re.test('' + h))
									h = Math.max(parseInt(h)
											+ (o.deltaHeight || 0), 100);
								o = t.theme.renderUI({
									targetNode : e,
									width : w,
							...
							if (tinymce.relaxedDomain)
								t.iframeHTML += '<script type="text/javascript">document.domain = "'
										+ tinymce.relaxedDomain + '";</script>';
							...
							t.iframeHTML += '</head><body id="' + bi
									+ '" class="mceContentBody ' + bc
									+ '"></body></html>';
							if (tinymce.relaxedDomain) {
								if (isIE
										|| (tinymce.isOpera && parseFloat(opera
												.version()) >= 9.5))
									u = 'javascript:(function(){document.open();document.domain="'
											+ document.domain
						...
						setupIframe : function() {
							var t = this, s = t.settings, e = DOM.get(t.id), d = t
									.getDoc(), h, b;
							if (!isIE || !tinymce.relaxedDomain) {
								d.open();
							...
							t.serializer = new tinymce.dom.Serializer(
									{
										...
										entities : s.entities,
										valid_elements : s.verify_html === false ? '*[*]'
												: s.valid_elements,
							...
							t.selection = new tinymce.dom.Selection(t.dom, t
									.getWin(), t.serializer);
							...
							if (!s.readonly)
								t._addEvents();
							t.controlManager.onPostRender.dispatch(t,
									t.controlManager);
							...
							if (s.handle_node_change_callback) {
								t.onNodeChange.add(function(ed, cm, n) {
									t.execCallback(
											...
											t.id, n, -1, -1, true, t.selection
													.isCollapsed());
								});
							}
							if (s.save_callback) {
								t.onSaveContent.add(function(ed, o) {
									var h = t.execCallback('save_callback',
											t.id, o.content, t.getBody());
									if (h)
										o.content = h;
								});
							}
								...
								t.onPostProcess
										.add(function(ed, o) {
											...
											o.content = o.content.replace(
													/<\/pre>\s*$/, '');
											if (o.set)
												o.content = '<pre class="mceItemHidden">'
														+ o.content + '</pre>';
										});
							}
							...
							if (s.cleanup_callback) {
								...
								t.onPreProcess.add(function(ed, o) {
									if (o.set)
										t.execCallback('cleanup_callback',
												'insert_to_editor_dom', o.node,
												o);
									if (o.get)
										t.execCallback('cleanup_callback',
												'get_from_editor_dom', o.node,
												o);
								});
								t.onPostProcess.add(function(ed, o) {
									if (o.set)
							...
							if (s.handle_event_callback) {
								t.onEvent.add(function(ed, e, o) {
									if (t.execCallback('handle_event_callback',
											e, ed, o) === false)
										Event.cancel(e);
								});
							}
							...
							setTimeout(function() {
								...
								t.load({
									initial : true,
									format : (s.cleanup_on_startup ? 'html'
											: 'raw')
								});
								...
								t.initialized = true;
								t.onInit.dispatch(t);
								t.execCallback('setupcontent_callback', t.id, t
										.getBody(), t.getDoc());
								...
								if (s.content_css) {
									tinymce.each(explode(s.content_css),
											function(u) {
												t.dom.loadCSS(t.documentBaseURI
														.toAbsolute(u));
											});
								}
								if (s.auto_focus) {
									setTimeout(function() {
										var ed = EditorManager
												.get(s.auto_focus);
						...
						focus : function(sf) {
							...
							if (!sf) {
								if (!ce
										&& (!isIE || t.selection.getNode().ownerDocument != t
												.getDoc()))
									t.getWin().focus();
							}
						...
						execCallback : function(n) {
							...
							return f.apply(s || t, Array.prototype.slice.call(
									arguments, 1));
						},
						...
						getLang : function(n, dv) {
							return EditorManager.i18n[(this.settings.language || 'en')
									+ '.' + n]
									|| (is(dv) ? dv : '{#' + n + '}');
						},
						getParam : function(n, dv, ty) {
							var tr = tinymce.trim, v = is(this.settings[n]) ? this.settings[n]
									: dv, o;
							if (ty === 'hash') {
								o = {};
								if (is(v, 'string')) {
						...
						nodeChanged : function(o) {
							var t = this, s = t.selection, n = s.getNode()
									|| t.getBody();
							if (t.initialized) {
								t.onNodeChange
										.dispatch(t, o ? o.controlManager
												|| t.controlManager
												: t.controlManager, isIE
												&& n.ownerDocument != t
														.getDoc() ? t.getBody()
												: n, s.isCollapsed(), o);
							}
							...
							each(explode(pa), function(pa) {
								...
								each(explode(pa, '+'), function(v) {
									...
									default:
										...
										o.keyCode = v.toUpperCase().charCodeAt(
												0);
									}
								});
								t.shortcuts[(o.ctrl ? 'ctrl' : '') + ','
										+ (o.alt ? 'alt' : '') + ','
										+ (o.shift ? 'shift' : '') + ','
										+ o.keyCode] = o;
							});
						...
						execCommand : function(cmd, ui, val, a) {
							...
							if (o = t.execCommands[cmd]) {
								...
								if (st !== true) {
									t.onExecCommand
											.dispatch(t, cmd, ui, val, a);
									return st;
								}
							}
							each(t.plugins, function(p) {
								if (p.execCommand
										&& p.execCommand(cmd, ui, val)) {
									t.onExecCommand
											.dispatch(t, cmd, ui, val, a);
									s = 1;
									return false;
							...
							if (s)
								return true;
							if (t.theme.execCommand
									&& t.theme.execCommand(cmd, ui, val)) {
								t.onExecCommand.dispatch(t, cmd, ui, val, a);
								return true;
							}
						...
						resizeToContent : function() {
							var t = this;
							DOM.setStyle(t.id + "_ifr", 'height',
									t.getBody().scrollHeight);
						},
						load : function(o) {
							...
							if (e) {
								o = o || {};
								o.load = true;
								h = t.setContent(is(e.value) ? e.value
										: e.innerHTML, o);
								o.element = e;
								if (!o.no_events)
						...
						setContent : function(h, o) {
							...
							o.content = t.dom.setHTML(t.getBody(), tinymce
									.trim(o.content));
						...
						isDirty : function() {
							var t = this;
							return tinymce.trim(t.startContent) != tinymce
									.trim(t.getContent({
										format : 'raw',
										no_events : 1
									}))
						...
						getContainer : function() {
							var t = this;
							if (!t.container)
								t.container = DOM.get(t.editorContainer || t.id
										+ '_parent');
							return t.container;
						},
						...
						convertURL : function(u, n, e) {
							...
							if (s.urlconverter_callback)
								return t.execCallback('urlconverter_callback',
										u, e, true, n);
							...
							u = t.documentBaseURI.toAbsolute(u,
									s.remove_script_host);
							return u;
						},
						...
						_addEvents : function() {
							...
							function eventHandler(e, o) {
								...
								if (t.onEvent.dispatch(t, e, o) !== false) {
									t[lo[e.fakeType || e.type]].dispatch(t, e,
											o);
								}
							}
							;
							each(
									...
									function(v, k) {
										...
										case 'submit':
										case 'reset':
											Event.add(t.getElement().form
													|| DOM.getParent(t.id,
															'form'), k,
													eventHandler);
											break;
										default:
											Event.add(s.content_editable ? t
													.getBody() : t.getDoc(), k,
													eventHandler);
										}
									});
							Event.add(s.content_editable ? t.getBody()
									: (isGecko ? t.getDoc() : t.getWin()),
									'focus', function(e) {
										t.focus(true);
									});
							if (tinymce.isGecko) {
							...
							if (s.custom_shortcuts) {
								if (s.custom_undo_redo_keyboard_shortcuts) {
									t.addShortcut('ctrl+z', t
											.getLang('undo_desc'), 'Undo');
									t.addShortcut('ctrl+y', t
											.getLang('redo_desc'), 'Redo');
								}
								if (isGecko) {
									t.addShortcut('ctrl+b', t
											.getLang('bold_desc'), 'Bold');
									t.addShortcut('ctrl+i', t
											.getLang('italic_desc'), 'Italic');
									t.addShortcut('ctrl+u', t
								...
								t.addShortcut('ctrl+7', '', [ 'FormatBlock',
										false, '<p>' ]);
								t.addShortcut('ctrl+8', '', [ 'FormatBlock',
										false, '<div>' ]);
								t.addShortcut('ctrl+9', '', [ 'FormatBlock',
										false, '<address>' ]);
								function find(e) {
									var v = null;
						...
						_convertInlineElements : function() {
							...
							function convert(ed, o) {
								...
								if (o.get) {
									each(
											t.dom.select('table,u,strike',
													o.node),
											function(n) {
												switch (n.nodeName) {
												case 'TABLE':
													if (v = dom.getAttrib(n,
															'height')) {
														dom.setStyle(n,
																'height', v);
														dom.setAttrib(n,
												...
												case 'U':
												case 'STRIKE':
													n.style.textDecoration = n.nodeName == 'U' ? 'underline'
															: 'line-through';
													dom.setAttrib(n,
															'mce_style', '');
													dom.setAttrib(n,
									...
									each(
											t.dom.select('table,span', o.node)
													.reverse(),
											function(n) {
												if (n.nodeName == 'TABLE') {
													if (v = dom.getStyle(n,
						...
						_convertFonts : function() {
							...
							fzn = [ 'xx-small', 'x-small', 'small', 'medium',
							...
							function process(no) {
								...
								for (x = nl.length - 1; x >= 0; x--) {
									...
									if (n.size) {
										if (sl)
											dom.setStyle(sp, 'fontSize',
													sl[parseInt(n.size) - 1]);
										else
											dom.setAttrib(sp, 'class',
													cl[parseInt(n.size) - 1]);
									}
...
(function() {
	...
	function isBlock(n) {
		return /^(H[1-6]|HR|P|DIV|ADDRESS|PRE|FORM|TABLE|OL|UL|TD|CAPTION|BLOCKQUOTE|CENTER|DL|DT|DD|DIR|FIELDSET|NOSCRIPT|NOFRAMES|MENU|ISINDEX|SAMP)$/
				.test(n.nodeName);
	}
	;
	tinymce
			.create(
					'tinymce.EditorCommands',
					{
						...
						execCommand : function(cmd, ui, val) {
							...
							case 'JustifyRight':
							case 'JustifyFull':
								t.mceJustify(cmd, cmd.substring(7)
										.toLowerCase());
								return true;
										...
										function(e) {
											d
													.setStyle(
															e,
															'paddingLeft',
															(parseInt(e.style.paddingLeft || 0) + iv)
																	+ iu);
										});
								return;
							}
						...
						Outdent : function() {
							...
							if (ed.settings.inline_styles
								...
								each(this._getSelectedBlocks(), function(e) {
									v = Math.max(0,
											parseInt(e.style.paddingLeft || 0)
													- iv);
						...
						mceSetAttribute : function(u, v) {
							...
							if (e = d.getParent(ed.selection.getNode(),
									d.isBlock))
						...
						mceReplaceContent : function(u, v) {
							...
							s.setContent(v.replace(/\{\$selection\}/g, s
									.getContent({
										format : 'text'
									})));
						},
						mceInsertLink : function(u, v) {
							var ed = this.editor, s = ed.selection, e = ed.dom
									.getParent(s.getNode(), 'A');
							...
							;
							if (!e) {
								ed.execCommand('CreateLink', false,
										'javascript:mctmp(0);');
						...
						FontName : function(u, v) {
							...
							if (!v) {
								...
								if (ed.settings.convert_fonts_to_spans)
									t._applyInlineStyle('span', {
										style : {
											fontFamily : v
										}
									});
								else
									ed.getDoc().execCommand('FontName', false,
											v);
							}
						},
						...
						queryCommandState : function(cmd) {
							...
							case 'JustifyRight':
							case 'JustifyFull':
								return this.queryStateJustify(cmd, cmd
						...
						queryValueFontSize : function() {
							...
							if (p = ed.dom.getParent(ed.selection.getNode(),
									'SPAN'))
								v = p.style.fontSize;
							if (!v && (isOpera || isWebKit)) {
								if (p = ed.dom.getParent(
										ed.selection.getNode(), 'FONT'))
									v = p.size;
								return v;
							}
						...
						queryValueFontName : function() {
							...
							if (p = ed.dom.getParent(ed.selection.getNode(),
									'SPAN'))
								v = p.style.fontFamily.replace(/, /g, ',')
						...
						mceJustify : function(c, v) {
							var ed = this.editor, se = ed.selection, n = se
									.getNode(), nn = n.nodeName, bl, nb, dom = ed.dom, rm;
							if (ed.settings.inline_styles
									&& this.queryStateJustify(c, v))
								rm = 1;
							...
							if (ed.settings.inline_styles
								...
								if (rm)
									v = '';
								each(
										this._getSelectedBlocks(dom.getParent(
												se.getStart(), dom.isBlock),
												dom.getParent(se.getEnd(),
														dom.isBlock)),
										function(e) {
							...
							if (ed.settings.inline_styles) {
								if (rm) {
									dom.getParent(ed.selection.getNode(),
											function(n) {
												if (n.style
														&& n.style.textAlign)
													dom.setStyle(n,
						...
						getSelectedElement : function() {
							var t = this, ed = t.editor, dom = ed.dom, se = ed.selection, r = se
									.getRng(), r1, r2, sc, ec, so, eo, e, sp, ep, re;
							...
							if (isIE) {
								...
								if (sc == ec) {
									...
									if (r1.compareEndPoints('StartToStart', r) == 0
											&& r1.compareEndPoints('EndToEnd',
													r) == 0)
										return re && re.test(sc.nodeName) ? null
												: sc;
								}
							} else {
								...
								if (sp == ep)
									return re && sp && re.test(sp.nodeName) ? null
											: sp;
							}
							return null;
						},
						InsertHorizontalRule : function() {
							if (isGecko || isIE)
								...
								this.editor.getDoc().execCommand(
						...
						RemoveFormat : function() {
							...
							if (isWebKit)
								...
								ed.getDoc().execCommand('RemoveFormat', false,
										null);
							t.mceSetStyleInfo(0, {
						...
						mceSetStyleInfo : function(u, v) {
							var t = this, ed = t.editor, d = ed.getDoc(), dom = ed.dom, e, b, s = ed.selection, nn = v.wrapper
									|| 'span', b = s.getBookmark(), re;
							function set(n, e) {
								if (n.nodeType == 1) {
									switch (v.command) {
									case 'setattrib':
										return dom
												.setAttrib(n, v.name, v.value);
									case 'setstyle':
							...
							if ((e = t.getSelectedElement())
								...
								each(isWebKit ? dom.select('span') : dom
										.select('font'), function(n) {
									var sp, e;
									if (dom.getAttrib(n, 'face') == '__'
											|| n.style.fontFamily === '__') {
										sp = dom.create(nn, {
											mce_new : '1'
										});
										set(sp);
							...
							each(dom.select(nn).reverse(), function(n) {
								...
								if (!p || !dom.getAttrib(n, 'mce_new'))
									return;
								if (ed.settings.force_span_wrappers
										&& p.nodeName != 'SPAN')
									return;
								if (p.nodeName == nn.toUpperCase()
							...
							each(
									dom.select(nn).reverse(),
									function(n) {
										if (dom.getAttrib(n, 'mce_new')
												|| (dom.getAttribs(n).length <= 1 && n.className === '')) {
											if (!dom.getAttrib(n, 'class')
													&& !dom.getAttrib(n,
															'style'))
						...
						queryStateJustify : function(c, v) {
							...
							n = dom.getParent(ed.selection.getStart(),
									function(n) {
										return n.nodeType == 1
												&& n.style.textAlign;
									});
						...
						FormatBlock : function(ui, val) {
							...
							if (bl) {
								if ((isIE && isBlock(bl.parentNode))
										|| bl.nodeName == 'DIV') {
									nb = ed.dom.create(val);
									each(dom.getAttribs(bl), function(v) {
										dom.setAttrib(nb, v.nodeName, dom
												.getAttrib(bl, v.nodeName));
									});
									b = s.getBookmark();
									dom.replace(nb, bl, 1);
									s.moveToBookmark(b);
							...
							val = ed.settings.forced_root_block ? (val || '<p>')
									: val;
							if (val.indexOf('<') == -1)
								val = '<' + val + '>';
							if (tinymce.isGecko)
						...
						mceCleanup : function() {
							var ed = this.editor, s = ed.selection, b = s
									.getBookmark();
							ed.setContent(ed.getContent());
							s.moveToBookmark(b);
						},
						mceRemoveNode : function(ui, val) {
							var ed = this.editor, s = ed.selection, b, n = val
									|| s.getNode();
							if (n == ed.getBody())
								return;
							b = s.getBookmark();
						...
						mceInsertRawHTML : function(ui, val) {
							...
							ed.setContent(ed.getContent().replace(
									/tiny_mce_marker/g, val));
						},
						mceRepaint : function() {
						...
						queryStateOutdent : function() {
							...
							if (ed.settings.inline_styles) {
								if ((n = ed.dom.getParent(ed.selection
										.getStart(), ed.dom.isBlock))
										&& parseInt(n.style.paddingLeft) > 0)
									return true;
								if ((n = ed.dom.getParent(
										ed.selection.getEnd(), ed.dom.isBlock))
										&& parseInt(n.style.paddingLeft) > 0)
									return true;
							}
							return this.queryStateInsertUnorderedList()
									|| this.queryStateInsertOrderedList()
									|| (!ed.settings.inline_styles && !!ed.dom
											.getParent(ed.selection.getNode(),
													'BLOCKQUOTE'));
						},
						...
						mceBlockQuote : function() {
							...
							if (sb != eb
									...
									|| (sb.childNodes.length == 1 && sb.firstChild.nodeName != 'BR'))
								bm = s.getBookmark();
							each(t._getSelectedBlocks(getBQ(s.getStart()),
									getBQ(s.getEnd())), function(e) {
						...
						_applyInlineStyle : function(na, at, op) {
							...
							function replaceFonts() {
								var bm;
								each(
										dom.select(tinymce.isWebKit
												&& !tinymce.isAir ? 'span'
												: 'font'),
										function(n) {
											if (n.style.fontFamily == 'mceinline'
													|| n.face == 'mceinline') {
												if (!bm)
													bm = ed.selection
															.getBookmark();
												at._mce_new = '1';
												dom.replace(dom.create(na, at),
														n, 1);
											}
										});
								each(
							...
							if (kh = t._applyInlineStyle.keyhandler) {
								...
								ed.onSetContent
										.remove(t._applyInlineStyle.chandler);
							}
							if (ed.selection.isCollapsed()) {
								t._pendingStyles = tinymce.extend(
										t._pendingStyles || {}, at.style);
								t._applyInlineStyle.chandler = ed.onSetContent
										.add(function() {
											delete t._pendingStyles;
										});
								t._applyInlineStyle.keyhandler = kh = function(
										e) {
									if (t._pendingStyles) {
									...
									if (replaceFonts()) {
										ed.onKeyDown
												.remove(t._applyInlineStyle.keyhandler);
										ed.onKeyPress
												.remove(t._applyInlineStyle.keyhandler);
									}
									if (e.type == 'keyup')
		...
		.create(
				'tinymce.UndoManager',
				{
					...
					add : function(l) {
						...
						la = t.data[t.index > 0
								&& (t.index == 0 || t.index == t.data.length) ? t.index - 1
								: t.index];
	...
	tinymce
			.create(
					'tinymce.ForceBlocks',
					{
						ForceBlocks : function(ed) {
							...
							t.reBR2Nbsp = new RegExp(
									'<p( )([^>]+)>\\s*<br \\\/>\\s*<\\\/p>|<p>\\s*<br \\\/>\\s*<\\\/p>'
											.replace(/p/g, elm), 'gi');
							t.reTrailBr = new RegExp('\\s*<br \\/>\\s*<\\\/p>'
									.replace(/p/g, elm), 'gi');
							function padd(ed, o) {
								if (isOpera)
									o.content = o.content.replace(t.reOpera,
											'</' + elm + '>');
								o.content = o.content.replace(t.rePadd, '<'
										+ elm + '$1$2$3$4$5$6>\u00a0</' + elm
										+ '>');
								if (!isIE && !isOpera && o.set) {
									o.content = o.content.replace(t.reNbsp2BR1,
											'<' + elm + '$1$2><br /></' + elm
													+ '>');
									o.content = o.content.replace(t.reNbsp2BR2,
											'<' + elm + '$1$2><br /></' + elm
													+ '>');
								} else {
									o.content = o.content.replace(t.reBR2Nbsp,
											'<' + elm + '$1$2>\u00a0</' + elm
													+ '>');
									o.content = o.content.replace(t.reTrailBr,
											'</' + elm + '>');
								}
							}
							;
						...
						setup : function() {
							...
							function ren(rn, na) {
								...
								each(rn.attributes, function(a) {
									if (a.specified && a.nodeValue)
										ne.setAttribute(a.nodeName
												.toLowerCase(), a.nodeValue);
								});
							...
							if (isIE && s.element != 'P') {
								...
								ed.onKeyUp.add(function(ed, e) {
									var bl, sel = ed.selection, n = sel
											.getNode(), b = ed.getBody();
						...
						find : function(n, t, s) {
							var ed = this.editor, w = ed.getDoc()
									.createTreeWalker(n, 4, null, false), c = -1;
							while (n = w.nextNode()) {
								c++;
								if (t == 0 && n == s)
									return c;
						...
						forceRoots : function(ed, e) {
							var t = this, ed = t.editor, b = ed.getBody(), d = ed
									.getDoc(), se = ed.selection, s = se
									.getSel(), r = se.getRng(), si = -2, ei, so, eo, tr, c = -0xFFFFFF;
							var nx, bl, bp, sp, le, nl = b.childNodes, i, n, eid;
							for (i = nl.length - 1; i >= 0; i--) {
								nx = nl[i];
								if (nx.nodeType == 3
										|| (!t.dom.isBlock(nx) && nx.nodeType != 8)) {
									if (!bl) {
										if (nx.nodeType != 3
												|| /[^\s]/g.test(nx.nodeValue)) {
											if (si == -2 && r) {
												if (!isIE) {
											...
											bl = ed.dom
													.create(ed.settings.forced_root_block);
							...
							if (si != -2) {
								if (!isIE) {
									bl = b
											.getElementsByTagName(ed.settings.element)[0];
									r = d.createRange();
									if (si != -1)
						...
						insertPara : function(e) {
							var t = this, ed = t.editor, dom = ed.dom, d = ed
									.getDoc(), se = ed.settings, s = ed.selection
									.getSel(), r = s.getRangeAt(0), b = d.body;
							var rb, ra, dir, sn, so, en, eo, sb, eb, bn, bef, aft, sc, ec, n, vp = dom
									.getViewPort(ed.getWin()), y, ch, car;
							function isEmpty(n) {
								n = n.innerHTML;
							...
							if (t.dom.getParent(sb, function(n) {
								...
								return true;
							if (sb
									&& (sb.nodeName == 'CAPTION' || /absolute|relative|static/gi
											.test(sb.style.position))) {
								bn = se.element;
								sb = null;
							}
							if (eb
									&& (eb.nodeName == 'CAPTION' || /absolute|relative|static/gi
											.test(eb.style.position))) {
								bn = se.element;
								eb = null;
							}
							if (/(TD|TABLE|TH|CAPTION)/.test(bn)
									|| (sb && bn == "DIV" && /left|right/gi
											.test(sb.style.cssFloat))) {
								bn = se.element;
								sb = eb = null;
							}
							bef = (sb && sb.nodeName == bn) ? sb.cloneNode(0)
									: ed.dom.create(bn);
							aft = (eb && eb.nodeName == bn) ? eb.cloneNode(0)
									: ed.dom.create(bn);
							aft.removeAttribute('id');
							if (/^(H[1-6])$/.test(bn) && sn.nodeValue
							...
							do {
								if (n == b
									...
									break;
								sc = n;
							} while ((n = n.previousSibling ? n.previousSibling
									: n.parentNode));
							n = ec = en;
							do {
								if (n == b
									...
									break;
								ec = n;
							} while ((n = n.nextSibling ? n.nextSibling
									: n.parentNode));
							if (sc.nodeName == bn)
								rb.setStart(sc, 0);
							else
							...
							if (!sc.previousSibling
									&& sc.parentNode.nodeName == bn) {
								r.setStartBefore(sc.parentNode);
							} else {
								if (rb.startContainer.nodeName == bn
										&& rb.startOffset == 0)
							...
							;
							r = d.createRange();
							r.selectNodeContents(isGecko ? first(car || aft)
									: car || aft);
							r.collapse(1);
							s.removeAllRanges();
							s.addRange(r);
						...
						backspaceDelete : function(e, bs) {
							var t = this, ed = t.editor, b = ed.getBody(), n, se = ed.selection, r = se
									.getRng(), sc = r.startContainer, n, w, tn;
							if (sc && ed.dom.isBlock(sc)
									&& !/^(TD|TH)$/.test(sc.nodeName) && bs) {
								if (sc.childNodes.length == 0
									...
									if (n) {
										if (sc != b.firstChild) {
											w = ed.dom.doc.createTreeWalker(n,
											...
											r
													.setStart(
															n,
															n.nodeValue ? n.nodeValue.length
																	: 0);
											r
													.setEnd(
															n,
							...
							function handler(e) {
								var pr;
								e = e.target;
								if (e && e.parentNode && e.nodeName == 'BR'
	...
	tinymce
			.create(
					'tinymce.WindowManager',
					{
						...
						open : function(s, p) {
							var t = this, f = '', x, y, mo = t.editor.settings.dialog_type == 'modal', w, sw, sh, vp = tinymce.DOM
									.getViewPort(), u;
							s = s || {};
							p = p || {};
							...
							s.resizable = true;
							s.left = s.left || parseInt(sw / 2.0)
									- (s.width / 2.0);
							s.top = s.top || parseInt(sh / 2.0)
									- (s.height / 2.0);
							p.inline = false;
							p.mce_width = s.width;
						...
						confirm : function(t, cb, s, w) {
							w = w || window;
							cb.call(s || this, w.confirm(this
									._decode(this.editor.getLang(t, t))));
						},
